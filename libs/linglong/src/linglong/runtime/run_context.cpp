...
bool allowHostRootAccess(const json &root, const std::string &appId)
{
    ...
    if (it == root.end()) {
        // Default-on for host root exposure to keep /run/host/rootfs available
        // without requiring extra whitelist configuration.
        return true;
    }
    if (it->is_boolean()) {
        return it->get<bool>();
...
bool bindDynamicDevRoot(generator::ContainerCfgBuilder &builder)
{
    const std::filesystem::path devRoot = "/dev";
    std::error_code ec;
    if (!std::filesystem::exists(devRoot, ec)) {
        if (ec) {
            LogW("skip dynamic /dev mount: {}", ec.message());
        }
        return false;
    }

    ocppi::runtime::config::types::Mount mount{
        .destination = devRoot.string(),
        .options = std::vector<std::string>{ "rbind", "rslave" },
        .source = devRoot.string(),
        .type = "bind",
    };
    builder.addExtraMount(std::move(mount));
    return true;
...
collectPackagedUdevRules(const std::filesystem::path &rulesDir)
{
    std::vector<std::pair<std::string, std::string>> rules;
    std::error_code ec;
    if (!std::filesystem::exists(rulesDir, ec) || !std::filesystem::is_directory(rulesDir, ec)) {
        return rules;
    }
    ...
    for (const auto &entry : std::filesystem::directory_iterator(rulesDir, ec)) {
        ...
        ruleFiles.push_back(std::move(filePath));
    }
    std::sort(ruleFiles.begin(), ruleFiles.end());

    for (const auto &rulePath : ruleFiles) {
        std::ifstream in(rulePath);
        if (!in.is_open()) {
            LogW("failed to read packaged udev rule {}", rulePath.string());
            continue;
        }
        ...
        rules.emplace_back(rulePath.filename().string(), std::move(content));
    }
...
std::vector<std::pair<std::string, std::string>>
collectMergedUdevRules(const json &root,
                       const std::optional<std::filesystem::path> &packagedRulesDir)
{
    std::vector<std::pair<std::string, std::string>> rules;
    if (packagedRulesDir) {
        auto packagedRules = collectPackagedUdevRules(*packagedRulesDir);
        rules.insert(rules.end(),
                     std::make_move_iterator(packagedRules.begin()),
                     std::make_move_iterator(packagedRules.end()));
    }

    auto customRules = collectCustomUdevRules(root);
    rules.insert(rules.end(),
                 std::make_move_iterator(customRules.begin()),
                 std::make_move_iterator(customRules.end()));
    return rules;
}
...
bool syncCustomUdevRules(const std::vector<std::pair<std::string, std::string>> &rules,
                            ...
                            const std::optional<std::filesystem::path> &packagedRulesDir)
{
    ...
    auto allRules = collectMergedUdevRules(mergedCfg, packagedRulesDir);
    bool hasCustomUdevRules = syncCustomUdevRules(allRules, customDir);
    bool dynamicUdev = hasCustomUdevRules;
    bool useUdev = enabled.count("udev") > 0 || hasCustomUdevRules;

    if (enabled.empty() && !useUdev) {
        return;
    }

    if (dynamicUdev) {
        if (bindDynamicDevRoot(builder)) {
            environment["LINGLONG_UDEV_DYNAMIC_DEV"] = "1";
        } else {
            LogW("custom udev rules are enabled but /dev cannot be mounted dynamically");
        }
    }

    if (enabled.count("usb") > 0 && !dynamicUdev) {
        bindPath(builder, "/dev/bus/usb", "/dev/bus/usb", true, false);
    }
    if (enabled.count("usb-hid") > 0 && !dynamicUdev) {
...
detectAppPackagedUdevRulesDir(const std::optional<RuntimeLayer> &appLayer)
{
    if (!appLayer || !appLayer->getLayerDir()) {
        return std::nullopt;
    }

    auto rulesDir = appLayer->getLayerDir()->filesDirPath() / "etc/udev/rules.d";
    std::error_code ec;
    if (!std::filesystem::exists(rulesDir, ec) || !std::filesystem::is_directory(rulesDir, ec)) {
        return std::nullopt;
    }
...
std::vector<std::string> commandMatchKeys(const std::optional<std::string> &execName,
                                          const std::string &appId)
{
    ...
    if (execName && !execName->empty()) {
        append(*execName);
        append(std::filesystem::path(*execName).filename().string());
    }
    append(appId);
    append("*");
    return keys;
...
const json *pickCommandNode(const json &merged, const std::vector<std::string> &keys)
{
    ...
    for (const auto &key : keys) {
        if (auto it = cmds.find(key); it != cmds.end() && it->is_object()) {
            return &(*it);
        }
    }
    return nullptr;
...
RunContext::CommandSettings parseCommandSettings(const std::string &appId, const json &node)
{
    ...
    if (node.contains("entrypoint") && node.at("entrypoint").is_string()) {
        auto entrypoint = node.at("entrypoint").get<std::string>();
        if (entrypoint.find('$') != std::string::npos) {
            qWarning() << "ignore entrypoint with variable expansion in command settings";
        } else {
            cs.entrypoint = expandUserHome(entrypoint);
        }
    }
    if (node.contains("cwd") && node.at("cwd").is_string()) {
        auto cwd = node.at("cwd").get<std::string>();
        if (cwd.find('$') != std::string::npos) {
            qWarning() << "ignore cwd with variable expansion in command settings";
        } else {
            cs.cwd = expandUserHome(cwd);
        }
    }
    return cs;
}
...
utils::error::Result<void> RunContext::fillContextCfg(
  ...
  const std::optional<CommandSettings> &cmdSettings)
{
    ...
    if (runtimeConfigEnabled) {
        ...
        auto appPackagedUdevRulesDir = detectAppPackagedUdevRulesDir(appLayer);
        ...
        auto settings = cmdSettings ? cmdSettings : commandSettings();
        if (settings) {
...
std::optional<RunContext::CommandSettings> RunContext::commandSettings(
  const std::optional<std::string> &execName) const
{
    if (!runtimeConfigEnabled) {
        return std::nullopt;
    }
    ...
    auto keys = commandMatchKeys(execName, appId);
    const json *node = pickCommandNode(mergedCfg, keys);
    if (!node) {
        return std::nullopt;
    }
